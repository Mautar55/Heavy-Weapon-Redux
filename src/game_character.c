...
void CharacterUpdate() {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && gameTime - character0.lastFire >= character0.fire_frequency) {
        ...
        if (first_inactive >= 0) {
            arr_set(player_projectile_pool, first_inactive, new_proj);
        } else {
...
void TestFallingBombs() {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT) && IsKeyDown(KEY_Q) && gameTime - last_bomb_test_time >= 0.2) {
        ...
        Vector2 new_velocity = (Vector2){0,150};
        new_proj.velocity = new_velocity;//Vector2Add(new_velocity,(Vector2){vel/deltaTime,0});
...
void CharacterDraw() {
    ...
    for (int i = 0; i < enemy_bombs_pool.size; i++) {
        ...
        if (bomb->active) {
            ...
            // part of the texture
            const float bombLifetime = (float)(GetTime() - bomb->birth_time);
            const float animationLength = 1.0;
            const float animationDelta = Clamp(bombLifetime / animationLength,0.0f,0.9999f);
            const int spriteIndex = floor( animationDelta * 10);
            const Rectangle rectSrc = TrimmedFromBombAtlas(spriteIndex, texBomb);
            ...
            const float angle = 0;//RAD2DEG*Vector2Angle((Vector2){0,1},Vector2Normalize(bomb->velocity));
            // Origin of the texture (rotation/scale point), it's relative to destination rectangle size
            //Vector2 origin = { (float)frameWidth, (float)frameHeight };

            DrawTexturePro(texBomb, rectSrc, destRec, (Vector2){rectSrc.width/2.0,rectSrc.height/2.0}, angle, WHITE);
        }
